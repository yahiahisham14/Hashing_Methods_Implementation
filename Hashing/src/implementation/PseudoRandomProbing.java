...
public class PseudoRandomProbing<K,V> implements HashTable<K, V> {
	...
	//Array visited to keep track of visited elements
	boolean visited [];
	
	//pseudorandom array for generating random probing indexes
	...
	private int getProbe(K key, int iteration) {
		//get random step from the random ArrayList. 
		int home=getHash(key);
		int randomIndex=random.get(iteration%size);
		return Math.abs((home + randomIndex ) % size);
	}
	...
	private void updateSize() {
		...
		size *= 2;
		map = new ProbeBlock[size];
		//resize array random and reshuffle it.
		random=new ArrayList<Integer>();
		for (int i = 0; i < map.length; i++) {
			map[i]=new ProbeBlock(null, null);
			random.add(i);
	...
	private int getFreeIndex(K key) {
		...
		while (map[index].getKey() != null) {
			...
			if (index == homePlace && iteration != 0) {
				// for debugging.
//				System.out
//						.println("Error in pseudo Probing can't find a free space ");
				break;
			}
			collisions++;
			iteration++;
